...
const Step2: React.FC<{onComplete: () => void}> = ({ onComplete }) => {
    ...
    const handleSubmit = (e: FormEvent) => { e.preventDefault(); if (question && answer.trim()) onComplete(); };
    return (
         <form onSubmit={handleSubmit} className="space-y-4">
...
const ForgotStep1: React.FC<{ onComplete: (username: string) => void }> = ({ onComplete }) => {
    const [username, setUsername] = useState('');
    const handleSubmit = (e: FormEvent) => { e.preventDefault(); if (username.trim()) onComplete(username); }
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
...
const AuthButton: React.FC<{text: string; loadingText?: string; status?: LoadingState; onClick?: (e: React.MouseEvent<HTMLButtonElement>) => void}> = ({ text, loadingText, status='idle', onClick }) => (
     <button 
