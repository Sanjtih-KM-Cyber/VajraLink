...
const FirstLoginInfoView = ({ duressPassword, onContinue }) => {
    return (
...
const CredentialsStep = ({ onComplete, updateForm, data }) => {
...
const PersonnelDetailsStep = ({ onComplete, onBack, updateForm, data }) => {
    const isValid = data.serviceId && data.rank && data.unit && data.enlistmentDate && data.verifyingOfficer;
    const handleSubmit = (e) => { e.preventDefault(); if (isValid) onComplete(); };
...
const SecurityQuestionStep = ({ onComplete, onBack, updateForm, data }) => {
...
const BiometricStep = ({onComplete, onBack}) => {
    const [scanStatus, setScanStatus] = useState('idle');
    const [enrolled, setEnrolled] = useState({ standard: false, duress: false });
...
const ApprovalStep = ({ onComplete, data }) => {
...
const ForgotStep1 = ({ onComplete }) => {
    const [username, setUsername] = useState('');
    const handleSubmit = (e) => { e.preventDefault(); if (username) onComplete(username); }
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
