...
const weirderEncrypt = (text, internalKey) => {
...
const isPrintable = (str) => /^[\x20-\x7E\n\r\t]+$/.test(str);

const weirderDecrypt = (encrypted) => {
...
const OPSEC_CHECKS = [
  {
    ...
    getMessage: (match) => `DANGER: Direct mention of operational term "${match}". Rephrase immediately or use encryption.`
  },
  {
    type: 'regex',
    patterns: [/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i],
    getMessage: (match) => `Potential PII detected: Email address ("${match}"). Avoid sending personal contact information.`
  },
  {
    type: 'regex',
    patterns: [/\b(?:\+?1[ -]?)?\(?\d{3}\)?[ -.]?\d{3}[ -.]?\d{4}\b/],
    getMessage: (match) => `Potential PII detected: Phone number ("${match}"). Avoid sending personal contact information.`
  },
  {
    type: 'regex',
    patterns: [/\b\d{1,2}[A-Z]{3}\s?\d{4,10}\b/i],
    getMessage: (match) => `CRITICAL: Military Grid Reference ("${match}") detected. Use secure methods for coordinate sharing.`
  }
];
...
const analyzeDraft = (draft) => {
  for (const check of OPSEC_CHECKS) {
    if (check.type === 'keywords') {
      ...
      for (const keyword of check.patterns) {
...
const OpsecBotScreen = ({ chatInfo, onReportFiled }) => {
...
const CallScreen = ({ user, type, onEnd }) => {
...
const ChatScreen = ({ chatInfo, onHeaderClick, onReportFiled }) => {
  const [messages, setMessages] = useState([]);
  const [inputText, setInputText] = useState('');
  const [searchQuery, setSearchQuery] = useState('');
  const [opsecWarning, setOpsecWarning] = useState(null);
  ...
  const cipherPickerRef = useRef(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (cipherPickerRef.current && !cipherPickerRef.current.contains(event.target)) {
  ...
  const handleUnpin = (messageId) => setPinnedMessages(pinnedMessages.filter(p => p.id !== messageId));
  const handleEmojiSelect = (emoji) => setInputText(inputText + emoji);
